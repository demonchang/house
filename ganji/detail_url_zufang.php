...
foreach ($select_result as $key => $value) {
	...
	$content_fields = "select * from ganji_area where parent_id={$value['id']}";
	$select_results = $sql_class->querys($content_fields);
	foreach ($select_results as $k => $v) {
		$url = str_replace('ershoufang','zufang',$v['url']);
		//var_dump($key.$url);exit();
		...
		if(isset($out1[1]) && !empty($out1[1])){
			...
			file_put_contents('ganji_page_count.log',$url.' -lianjie'.PHP_EOL,FILE_APPEND);
			$page_count = 1;
		}
		//var_dump($page_count);exit();
		

		$content_fieldss = "update ganji_area set all_count_zufang={$all_count},page_count_zufang={$page_count} where id={$v['id']}";
		...
		for ($i=0; $i < $page_count; $i++) { 
			...
			preg_match_all('#class="dd-item title">[\s]*?<a.*?href="([\s\S]*?)"[\s\S]*?</a>#', $html, $out2);

			if(!isset($out2[1]) || empty($out2[1])){
				file_put_contents('ganji_url_list.log',$new_url.' -lianjie'.PHP_EOL,FILE_APPEND);
				continue;
			}
			//var_dump($out2[1]);exit();

			foreach ($out2[1] as $keys => $values) {
				$real_url = 'http:'.$values;
				$content_url_field = "select * from ganji_url_zufang where url='{$real_url}'";
				$select_result = $sql_class->querys($content_url_field);
				//var_dump($select_result);exit();
				if(empty($select_result)){
					$content_field = "insert into ganji_url_zufang(url,parent_id) values('{$real_url}',{$v['id']})";
