...
$content_field = "select * from lianjia_city where xiaoqu=1 ";
...
foreach ($select_result as $key => $value) {
	...
	foreach ($select_results as $k => $v) {
		$url = $base_url.str_replace('ershoufang','xiaoqu',$v['url']);
		//var_dump($key.' '.$url);
		...
		$content_fieldss = "update lianjia_area set all_count_xiaoqu={$all_count},page_count_xiaoqu={$page_count} where id={$v['id']}";
		...
		for ($i=0; $i < $page_count; $i++) { 
			...
			if(!isset($out2[1]) || empty($out2[1])){
				file_put_contents('lianjia_url_xiaoqu_list.log',$new_url.' -lianjie'.PHP_EOL,FILE_APPEND);
				continue;
			}
			//var_dump($out2[1]);exit();

			foreach ($out2[1] as $keys => $values) {
				$content_url_field = "select * from lianjia_url_xiaoqu where url='{$values}'";
				$select_result = $sql_class->querys($content_url_field);
				//var_dump($select_result);exit();
				if(empty($select_result)){
					$content_field = "insert into lianjia_url_xiaoqu(url,parent_id) values('{$values}',{$v['id']})";
