...
foreach ($select_result as $key => $value) {
	...
	foreach ($select_results as $k => $v) {
		...
		$content_fieldss = "update ganji_area set all_count={$all_count},page_count={$page_count} where id={$v['id']}";
		...
		for ($i=0; $i < $page_count; $i++) { 
			...
			foreach ($out2[1] as $keys => $values) {
				...
				$content_url_field = "select * from ganji_url where url='{$real_url}'";
				$select_result = $sql_class->querys($content_url_field);
				//var_dump($select_result);exit();
				if(empty($select_result)){
					$content_field = "insert into ganji_url(url,parent_id) values('{$real_url}',{$v['id']})";
