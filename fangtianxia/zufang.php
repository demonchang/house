...
while($select_result = $sql_class->querys("select * from fangtianxia_url_zufang where status=0 limit 100")){
	...
	foreach ($select_result as $key => $value) {
		...
		if(empty($html_city)){
			$status = 2; //内容抓取为空
			$content_fieldss = "update fangtianxia_url_zufang set status={$status} where id={$value['id']}";
		...
		preg_match('#<p class="gray9 fybh-zf">([\s\S]*?)</p>#', $html_city, $out1);
		...
		preg_match('#<div class="bqian clearfix">([\s\S]*?)</div>#', $html_city, $out2);
		...
		preg_match('#<div class="trl-item sty1">(.*?)</div>#', $html_city, $out3);
		...
		preg_match('#<div class="tt">([\S]*?)</div>[\s]*?<div class="font14">出租方式</div>#', $html_city, $out4);
		...
		preg_match('#<div class="tt">([\S]*?)</div>[\s]*?<div class="font14">户型</div>#', $html_city, $out4);
		if(!isset($out4[1]) || empty($out4[1])){
			$huxing = '';
		}else{
			$huxing = trim($out4[1]);
		...
		//var_dump($chaoxiang,$zhuangxiu,$mianji);exit();




		preg_match('#小&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;区([\s\S]*?)地&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;址#', $html_city, $out10);
		...
		preg_match('#地&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;址([\s\S]*?)</a></div>#', $html_city, $out11);
		...
		preg_match('#房源描述([\s\S]*?)配套设施#', $html_city, $out16);
		...
		preg_match('#配套设施([\s\S]*?)房源图片#', $html_city, $out18);
