...
foreach ($select_result as $key => $value) {
	...
	foreach ($select_results as $k => $v) {
		$url = str_replace('ershoufang','chuzu',$v['url']);
		//var_dump($key.$url);
		...
		$content_fieldss = "update wuba_area set all_count_zufang={$all_count},page_count_zufang={$page_count} where id={$v['id']}";
		...
		for ($i=0; $i < $page_count; $i++) { 
			...
			foreach ($out2[1] as $keys => $values) {
				$real_url = $values;
				$content_url_field = "select * from wuba_url_zufang where url='{$real_url}'";
				$select_result = $sql_class->querys($content_url_field);
				//var_dump($select_result);exit();
				if(empty($select_result)){
					$content_field = "insert into wuba_url_zufang(url,parent_id) values('{$real_url}',{$v['id']})";
